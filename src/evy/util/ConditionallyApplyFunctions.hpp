...
constexpr void conditionallyApplyFunction(
    InputType& input, 
    TupleLike auto& tupleOfFunctions, 
    std::invocable auto& condition)
{
  if(!condition())
  {
    return;
...
constexpr void conditionallyApplyFunctions(
    InputType& input, 
    TupleLike auto& tupleOfFunctions, 
    std::invocable auto& condition, 
...
constexpr void conditionallyApplyFunctions(
    InputType& input, 
    TupleOfFunctions& tupleOfFunctions, 
    std::invocable auto& condition) 
{
  if constexpr (TupleLike<InputType>)
  {
    ...
    constexpr auto tupleSize{ std::tuple_size_v<InputType> };
    detail::conditionallyApplyFunctions(
        input, 
        tupleOfFunctions, 
        condition, 
  ...
  else if constexpr (std::is_aggregate_v<InputType>)
  {
    ...
    constexpr auto tupleSize{ boost::pfr::tuple_size_v<InputType> };
    detail::conditionallyApplyFunctions(
        input, 
        tupleOfFunctions, 
        condition, 
